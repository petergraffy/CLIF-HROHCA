...
model_df <- model_df[model_df$denominator_icu_admissions > 0, , drop = FALSE]
...
formula_spec <- ohca_admissions ~ cb_temp +
  offset(log(denominator_icu_admissions)) +
...
surface <- expand.grid(
  ...
  mutate(
    ...
    export_family = "rate_dlnm",
    model = "rate_lag30_median_humidity_pollution_adjusted",
...
readr::write_csv(surface, file.path(output_dir, "rate_lag30_temperature_lag_rr_surface.csv"))
readr::write_csv(lag_specific_hot, file.path(output_dir, "rate_lag30_hot_temperature_lag_specific_rr.csv"))
readr::write_csv(cumulative_hot, file.path(output_dir, "rate_lag30_hot_temperature_cumulative_rr_by_lag.csv"))
readr::write_csv(temperature_distribution, file.path(output_dir, "rate_lag30_temperature_distribution_source.csv"))
readr::write_csv(diagnostic_summary, file.path(output_dir, "rate_lag30_diagnostic_summary.csv"))
...
figure_surface <- ggplot(surface, aes(x = .data$tmax_mean_c, y = .data$lag, fill = .data$rr)) +
  ...
  labs(
    title = "Thirty-day temperature-lag rate DLNM diagnostic surface",
    subtitle = "OHCA per ICU admission model; dashed horizontal line marks the primary 0-5 day lag window",
...
figure_cumulative <- ggplot(cumulative_hot, aes(x = .data$lag_end, y = .data$cumulative_rr)) +
  ...
  labs(
    title = "Cumulative hot-temperature relative rate by maximum lag",
    subtitle = sprintf("Hot Tmax %.1f C versus median reference %.1f C", hot_temp, reference_temp),
...
figure_temperature <- ggplot(temperature_distribution, aes(x = .data$tmax_mean_c)) +
  ...
  labs(
    title = "Temperature distribution used in 30-day rate lag diagnostic",
...
save_plot(figure_surface, "figure_rate_lag30_temperature_lag_rr_surface", width = 8, height = 5.8)
save_plot(figure_lag, "figure_rate_lag30_hot_temperature_lag_specific_rr", width = 7.5, height = 5.0)
save_plot(figure_cumulative, "figure_rate_lag30_hot_temperature_cumulative_rr_by_lag", width = 7.5, height = 5.0)
save_plot(figure_temperature, "figure_rate_lag30_temperature_distribution", width = 7.5, height = 4.5)

print(diagnostic_summary)
message("Wrote 30-day rate lag diagnostic plots and source files to ", output_dir, " and ", figure_dir)
